...
#define _SIZEOF_ADDR_IFREQ sizeof
    ...
    /* Win9x port: GetAdaptersInfo's own documented minimum supported
       client is Windows 2000 (learn.microsoft.com/.../
       nf-iphlpapi-getadaptersinfo) -- upstream's *static* import of it
       makes the whole DLL fail to load at all on Windows 95/98/ME, even
       though nothing in AmuletsArmor/AAServer/AALauncher ever calls this
       function. Resolve it dynamically instead: on 9x, iphlpapi.dll
       either won't be present or won't export this symbol, and this
       optional convenience function just returns 0 addresses instead of
    ...
    pGetAdaptersInfo = (T_GetAdaptersInfo)GetProcAddress(hIpHlpApi, "GetAdaptersInfo");
    ...
    if ((dwRetVal = pGetAdaptersInfo(pAdapterInfo, &ulOutBufLen)) == ERROR_BUFFER_OVERFLOW) {
